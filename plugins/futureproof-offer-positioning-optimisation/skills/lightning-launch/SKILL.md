...
FutureProof:save_session(skill="lightning-launch", session={
  summary: "Built Lightning Launch plan for [offer name] targeting [ICA segment] with [launch timeline] timeline and [revenue target] revenue goal",
  key_findings: ["finding 1: e.g. primary ICA objection is timing, not price", "finding 2: e.g. existing testimonials strongest on speed-to-result angle", "finding 3: e.g. competitive gap exists around guarantee specificity"],
  deliverables_produced: ["Positioning Brief", "Launch Execution Calendar", "Launch Copy Kit", "Launch Scorecard"],
  user_feedback: null
})
```