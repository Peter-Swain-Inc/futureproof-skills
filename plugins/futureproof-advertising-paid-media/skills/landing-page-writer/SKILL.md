...
FutureProof:save_session(skill="landing-page-writer", session={
  summary: "Wrote [page type] landing page for [offer/product] targeting [ICA segment] from [traffic source] at [awareness level] awareness",
  key_findings: ["finding 1: e.g., ICA language gap identified between ad copy and page headline", "finding 2: e.g., proof inventory thin — recommended collecting 3 additional results-based testimonials", "finding 3: e.g., awareness level required longer-form mechanism section than initially scoped"],
  deliverables: ["Landing Page Copy (production-ready)", "Landing Page Strategy Brief with score card", "Headline & CTA Variant Bank"],
  user_feedback: null
})
```