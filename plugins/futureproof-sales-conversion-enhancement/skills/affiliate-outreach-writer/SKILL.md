...
description: "Crafts high-converting affiliate recruitment and activation outreach sequences using FutureProof context."
---
...
FutureProof:save_session(skill="affiliate-outreach-writer", session={
  summary: "Crafted [X]-touch affiliate outreach sequence targeting [affiliate persona type] for [product/service] via [channel]",
  key_findings: ["finding 1: primary affiliate motivation identified as [motivation]", "finding 2: key objection to pre-empt is [objection]", "finding 3: recommended personalisation depth for this segment is [level]"],
  deliverables: ["Affiliate Outreach Sequence Document", "Affiliate Motivation Brief", "Performance Tracking Scorecard"],
  user_feedback: null
})
```