...
FutureProof:save_session(skill="beta-testing-plan-creator", session={
  summary: "Created beta testing plan for [product/feature name] targeting [ICA segment] across [number] phases over [duration]",
  key_findings: ["finding 1: e.g., ICA tester recruitment requires dedicated screening to avoid enthusiast bias", "finding 2: e.g., willingness-to-pay signal identified as critical gate metric given pricing uncertainty", "finding 3: e.g., Phase 0 dogfood stage added to mitigate reputational risk from P0 defects"],
  user_feedback: null
})
```