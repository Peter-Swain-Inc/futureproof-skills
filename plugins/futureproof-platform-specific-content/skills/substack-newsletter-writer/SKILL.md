...
FutureProof:save_session(skill="substack-newsletter-writer", session={
  summary: "Wrote [edition type] Substack newsletter on [topic] targeting [ICA segment], optimised for [primary objective]",
  key_findings: ["finding 1: e.g., ICA language gap identified in prior editions", "finding 2: e.g., paywall break placement recommendation based on content arc", "finding 3: e.g., subject line lever most aligned with audience psychology"],
  deliverables: ["Edition Strategy Brief", "3× Subject Line Options", "Full Newsletter Edition", "Platform Optimisation Notes"],
  user_feedback: null
})
```