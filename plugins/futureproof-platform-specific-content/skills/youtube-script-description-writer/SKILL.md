...
FutureProof:save_session(skill="youtube-script-description-writer", session={
  summary: "Created [deliverable scope] for [video topic] targeting [ICA segment] at [awareness level] awareness — [format type], [estimated duration]",
  key_findings: ["finding 1 — e.g., identified underserved angle on topic vs. top 5 competitors", "finding 2 — e.g., ICA language patterns suggest conversational register outperforms authoritative tone for this segment"],
  user_feedback: null
})
```