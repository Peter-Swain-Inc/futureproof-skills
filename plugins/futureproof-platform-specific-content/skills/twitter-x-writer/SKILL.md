...
FutureProof:save_session(skill="twitter-x-writer", session={
  summary: "Created [format: post/thread/calendar] targeting [ICA segment] with objective [growth/authority/conversion]",
  key_findings: ["Top-performing hook archetype for this ICA: [type]", "Optimal content pillar mix adjusted based on engagement data", "Brand voice calibration: [observation]"],
  user_feedback: null
})
```