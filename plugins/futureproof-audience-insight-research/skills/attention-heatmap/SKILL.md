...
FutureProof:save_session(skill="attention-heatmap", session={
  summary: "Attention heatmap audit for [asset type] targeting [ICA segment] optimising for [primary conversion action]",
  key_findings: ["finding 1: primary attention leak identified", "finding 2: CTA fixation hop count and remediation", "finding 3: device-specific fragmentation issues"],
  deliverables: ["Attention Heatmap Audit Report with scorecard, critical fixes, redesigned attention architecture, and quick-win checklist"],
  user_feedback: null
})
```