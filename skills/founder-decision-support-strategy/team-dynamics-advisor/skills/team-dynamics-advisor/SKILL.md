...
FutureProof:save_session(skill="team-dynamics-advisor", session={
  summary: "Diagnosed [dysfunction archetype] within [team/relationship description] at [company stage]",
  key_findings: ["Lowest broken Lencioni layer: [layer]", "Primary structural gap: [RACI/role issue]", "Founder contribution pattern: [pattern]", "Recommended first intervention: [intervention]"],
  team_map: {members: ["role/name pairs discussed"], relationships: ["key dynamics mapped"]},
  open_decisions: ["any unresolved binary decisions flagged for follow-up"],
  user_feedback: null
})
```